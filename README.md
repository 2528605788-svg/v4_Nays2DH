...
The workflow uses Intel Classic Fortran `ifort` 2023.1, the earliest release
whose official Intel download remains accessible to GitHub Actions. Upstream
pins `ifort` 2021.2, but Intel now returns HTTP 403 for that archived installer.
An A/B smoke test showed that an otherwise
unmodified upstream solver built with `ifx` 2026.1 overflowed in `HCAL` on its
first time step. A subsequent `ifort` 2024.2 build also stopped at time zero,
while the installed upstream executable advanced normally. Matching the
compiler compatibility is therefore part of preserving numerical behavior;
the vegetation equations were not the cause of the compiler-dependent failure.
